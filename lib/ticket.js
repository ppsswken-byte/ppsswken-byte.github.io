...
// ════════════════════════════════════════════════════
const TICKET_VERSION=3;   // HTML 側の ?v= と合わせる
...
const Printer={
  ...
  _send(build,kind){
    return new Promise(resolve=>{
      ...
      let done=false;
      const finish=(r,detail)=>{ if(done) return; done=true; this.busy=false; this.last={...r,kind,detail:detail||null,at:new Date()}; resolve(r); };
      ...
      try{
        ...
        tr.onReceive=res=>{
          ...
          if(ok) msg=why?'印刷しました（注意: '+why+'）':'印刷しました';
            ...
            // ステータスが取れない = アプリがプリンターにつながっていない
            msg='プリンターに接続できません（code '+res.traderCode+'）。iPad の設定 → webPRNT の PRINTER 設定（mC-Print2 / mC-Print2 (StarPRNT)）と、Bluetooth で「接続済み」になっているかを確認してください';
        ...
        tr.onError=res=>finish({ok:false,msg:'プリンターと通信できません（'+(res&&res.status)+'）Bluetooth接続とプリンターの電源を確認してください'},
          {http:res&&res.status,raw:raw(res&&res.responseText)});
        tr.onTimeout=()=>finish({ok:false,msg:'プリンターの応答がありません（タイムアウト）'},{raw:'timeout'});
